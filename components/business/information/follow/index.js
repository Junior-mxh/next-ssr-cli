...
const FansFollowList = dynamic(() => import('./fansfollow-list'))
const ActivityFollowList = dynamic(() => import('./activity-follow-list'))
const StudioFollowList = dynamic(() => import('./studio-follow-list'))
  ...
  render () {
    ...
    const ChildComponent = () => {
      const _ComponentOfKey = {
        default: () => <FansFollowList {...{data}}/>,
        activity: () => <ActivityFollowList />,
        studio: ()=> <StudioFollowList {...{data}}/>
      }
    ...
    const tabProps = {
      data: subTabList,
      render: tab => (
        <a href={`/information/${userId}/follow?type=${tab.value}`} key={tab.value}
...
export default connect(mapState, mapDispatch)(withRouter(Follow))
